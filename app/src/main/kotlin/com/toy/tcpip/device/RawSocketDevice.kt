...
/**
 * Implementation of NetworkDevice that uses Linux PF_PACKET sockets
 */
class RawSocketDevice(
    ...
    // Socket constants
    companion object {
        private const val PF_PACKET = 17
    ...
    override fun open() {
        ...
        // Create raw socket
        fd = cLibrary.socket(PF_PACKET, SOCK_RAW, (ETH_P_ALL).toShort().toInt())
        if (fd < 0) {
        ...
        try {
            ...
            val addr = SockAddrLl()
            addr.sll_family = PF_PACKET.toShort()
            addr.sll_protocol = 0 // We want all protocols
